...
Twitter / X 视频批量下载工具
用法：
  python scripts/twitter-vids-downloader.py            # 从 urls.txt 批量下载
  python scripts/twitter-vids-downloader.py <URL>      # 下载单个链接
"""
...
OUTPUT_DIR   = Path.home() / "Downloads" / "twitter-videos"
...
def load_urls(cli_args: list[str]) -> list[str]:
    ...
    if not URLS_FILE.exists():
        ...
        err("请创建 urls.txt，每行一个 Twitter/X 视频链接，或直接传入 URL 参数。")
        sys.exit(1)
...
def download(url: str, index: int, total: int) -> bool:
    ...
    if COOKIES_FILE.exists():
        ...
        warn("未找到 cookies.txt，将以未登录状态尝试下载（私密视频可能失败）。")
